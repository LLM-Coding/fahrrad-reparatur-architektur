...
Die konkreten Qualitätsszenarien vom Miro-Board (Seiten 5 und 6) stehen in <<section-quality-scenarios>>. Wie die Architektur die drei Ziele erreicht, beschreibt <<section-solution-strategy>>.
...
|Der Webshop bleibt unverändert und hält keine Reparaturdaten (<<adr-005>>). Das Reparatur-System hängt sich per URL-Umleitung von `/reparatur`, JWT-Cookie und REST an (<<adr-001>>, <<adr-002>>).
|===
